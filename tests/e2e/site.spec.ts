...
test('homepage presents broad positioning, navigation and working contacts', async ({ page }) => {
  ...
  await expect(
    page.getByRole('heading', {
      name: 'Создаю сайты и автоматизирую ручные процессы.',
    }),
  ).toBeVisible();
  await expect(page.getByRole('link', { name: 'Обсудить задачу' })).toHaveAttribute(
    'href',
  ...
  const casesLink = page.getByRole('link', { name: 'Автоматизация', exact: true }).first();
...
test('site catalogue exposes both web projects', async ({ page }) => {
  ...
  await expect(page.getByText('Реализованный демо-проект', { exact: true })).toHaveCount(2);
...
for (const [slug, title, liveUrl, adminUrl] of sites) {
  test(`site ${slug} exposes both live surfaces and honest limitations`, async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: title })).toBeVisible();
    await expect(page.getByText('Реализованный демо-проект', { exact: true })).toBeVisible();
